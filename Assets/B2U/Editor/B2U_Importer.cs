...
class B2U_Postprocessor : AssetPostprocessor {
    ...
    static void OnPostprocessAllAssets(string[] importedAssets, string[] deletedAssets, string[] movedAssets, string[] movedFromAssetPaths) {
        foreach (string str in importedAssets) {
            // Auto Import Groups
            if (str.Contains(".b2uc")) {
                B2U_Utils.CollectionToPrefab(str);
...
public class B2U_Utils {
    ...
    public static float parseFloatChannel(string sourceString) {
        ...
        splitString = sourceString.Split(","[0]);
        if(splitString.Length > 1) {
            outfloat = float.Parse(splitString[1], CultureInfo.InvariantCulture);
        }
        return outfloat;
    }
    ...
    public static Vector3 parseVector3(string sourceString) {
        ...
        outVector3.x = float.Parse(splitString[0], CultureInfo.InvariantCulture);
        outVector3.y = float.Parse(splitString[1], CultureInfo.InvariantCulture);
        outVector3.z = float.Parse(splitString[2], CultureInfo.InvariantCulture);
    ...
    public static Quaternion parseQuart(string sourceString) {
        ...
        outQuart.x = float.Parse(splitString[0], CultureInfo.InvariantCulture);
        outQuart.y = float.Parse(splitString[1], CultureInfo.InvariantCulture);
        outQuart.z = float.Parse(splitString[2], CultureInfo.InvariantCulture);
        outQuart.w = float.Parse(splitString[3], CultureInfo.InvariantCulture);
    ...
    public static void SceneToPrefab(string path) {
        ...
        foreach (XmlNode Obj in Objects) {
            ...
            if (Obj.Name == "Empty") {
                ...
                GameObject objTemp = new GameObject(nameObj);
        ...
        foreach (XmlNode Lamp in Lamps) {
            ...
            Color col;
            ColorUtility.TryParseHtmlString(Lamp.SelectSingleNode("Color").InnerText,out col);

            Debug.Log(colorLamp);
            Debug.Log(col);

            objLamp.GetComponent<Light>().color = col;
            float power = float.Parse(powerLamp, CultureInfo.InvariantCulture);
            objLamp.GetComponent<Light>().intensity = power;
            objLamp.GetComponent<Light>().range = power/3;
            ...
            if (typeLamp == "SPOT") {
                ...
                objLamp.GetComponent<Light>().spotAngle = float.Parse(SpotSize, CultureInfo.InvariantCulture);
        ...
        foreach (XmlNode Cam in Cams) {
            ...
            float vFov = Mathf.Atan(Mathf.Tan(Mathf.Deg2Rad * float.Parse(fovCam, CultureInfo.InvariantCulture) / 2) / objCam.GetComponent<Camera>().aspect) * 2;
            vFov *= Mathf.Rad2Deg;

            objCam.GetComponent<Camera>().fieldOfView = vFov;
            objCam.GetComponent<Camera>().nearClipPlane = float.Parse(nearCam, CultureInfo.InvariantCulture);
            objCam.GetComponent<Camera>().farClipPlane = float.Parse(farCam, CultureInfo.InvariantCulture);
            objCam.GetComponent<Camera>().orthographicSize = float.Parse(sizeCam, CultureInfo.InvariantCulture) * 0.28f;
        ...
        for (int k = 0; k < pList.Count; k++) {
            ...
            foreach (B2U_ParentList elem in pList) {
                if (Data.parentName == elem.obj.name) {
                    Debug.Log("Parenting:" + ObjSource.name + " with " + elem.obj.name);
                    ObjSource.transform.parent = elem.obj.transform;
                }
            }
        }
    ...
    public static void CollectionToPrefab(string path) {
        XmlDocument doc = new XmlDocument();
        try {
            doc.Load(path);
        }
        ...
        XmlNodeList Objects = root.SelectNodes("Empty");
        ...
        Objects = root.SelectNodes("Object");
        prefabPath = root.SelectSingleNode("Path").InnerText;
        foreach (XmlNode Obj in Objects) {
            Debug.Log("OBJ");
            string prefab_path = Obj.SelectSingleNode("Prefab").InnerText;
            string loc = Obj.SelectSingleNode("Position").InnerText;
